...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, translation=False):
  ...
  if sampler_name.lower() == 'rectified_flow':
    if translation:
      sampling_fn = get_translation_sampler(sde=sde, shape=shape, inverse_scaler=inverse_scaler, device=config.device)
    else:
...
def get_rectified_flow_sampler(sde, shape, inverse_scaler, device='cuda'):
  ...
  """
  def euler_sampler(model, context_condition, z=None):
    ...
    with torch.no_grad():
      ...
      for i in range(sde.sample_N):
        ...
        pred = model_fn(x, t*999, context_condition) ### Copy from models/utils.py 
  ...
  def rk45_sampler(model, context_condition, z=None):
    ...
    with torch.no_grad():
      ...
      def ode_func(t, x):
        ...
        drift = model_fn(x, vec_t*999, context_condition)
...
def get_translation_sampler(sde, shape, inverse_scaler, device='cuda'):
  """
  Get rectified flow sampler
  ...
  def rk45_sampler(model, cond_lst, z=None):
    """The probability flow ODE sampler with black-box ODE solver.
    ...
    with torch.no_grad():
      ...
      if z is None:
        ...
        x = z.to(device)
      label_s = torch.ones(shape[0]) * cond_lst[0]
      label_t = torch.ones(shape[0]) * cond_lst[1]
      ...
      def ode_func(t, x):
        x = from_flattened_numpy(x, shape).to(device).type(torch.float32)
        vec_t = torch.ones(shape[0], device=x.device) * t
        drift = model_fn(x, vec_t*999, label_t) - model_fn(x, vec_t*999, label_s)
        #drift = model_fn_2(x, vec_t*999)

        return to_flattened_numpy(drift)

      # Black-box ODE solver for the probability flow ODE
      solution = integrate.solve_ivp(ode_func, (eps, sde.T), to_flattened_numpy(x),
                                     rtol=rtol, atol=atol, method=method, t_eval=t_eval)
      nfe = solution.nfev
      #x = torch.tensor(solution.y[:, -1]).reshape(shape).to(device).type(torch.float32)
      result = []
      for i in range(len(t_eval)):
        x = torch.tensor(solution.y[:, i]).reshape(shape).to(device).type(torch.float32)
        x = inverse_scaler(x)
        result.append(x)
  ...
  '''
  def rk45_sampler(model1, model2, z=None):
    """The probability flow ODE sampler with black-box ODE solver.
    ...
    with torch.no_grad():
      ...
      model_fn_1 = mutils.get_model_fn(model1, train=False)
      model_fn_2 = mutils.get_model_fn(model2, train=False)

      def ode_func(t, x):
        x = from_flattened_numpy(x, shape).to(device).type(torch.float32)
        vec_t = torch.ones(shape[0], device=x.device) * t
        drift = model_fn_2(x, vec_t*999) - model_fn_1(x, vec_t*999)
        #drift = model_fn_2(x, vec_t*999)

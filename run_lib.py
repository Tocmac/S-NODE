...
def train(config, workdir):
  ...
  tb_dir = os.path.join(workdir, "tensorboard")  ### modify
  #tb_dir = '/root/tf-logs/'
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    batch = torch.from_numpy(batch_data['image']._numpy()).to(config.device).float()
    label = torch.from_numpy(batch_data['label']._numpy()).to(config.device).float()
    ...
    if step % config.training.eval_freq == 0:  ##### for debug
      eval_batch_data = next(eval_iter)
      eval_batch = torch.from_numpy(eval_batch_data['image']._numpy()).to(config.device).float()
      eval_label = torch.from_numpy(eval_batch_data['label']._numpy()).to(config.device).float()
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        context = [i % 10 for i in range(nrow)] * nrow
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_sampling:
      ...
      for r in range(num_sampling_rounds):
        ...
        tf.io.gfile.makedirs(this_sample_dir)
        context = torch.randint(0, 10, (config.eval.batch_size,)).to(config.device).float()
        samples, n = sampling_fn(score_model, context_condition=context)
      ...
      logging.info(
        "ckpt-%d --- inception_score: %.6e, FID: %.6e, KID: %.6e, NFE: %.4e" % (
          ckpt, inception_score, fid, kid, nfe_mean))

      with tf.io.gfile.GFile(os.path.join(eval_dir, f"report_{ckpt}.npz"),
                             "wb") as f:
        io_buffer = io.BytesIO()
        np.savez_compressed(io_buffer, IS=inception_score, fid=fid, kid=kid, nfe=nfe_mean)

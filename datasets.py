...
def get_pytorch_dataset(config, condition=None):
    import pytorch_datasets as tds
    import torchvision.transforms as tr
    if config.data.dataset == 'Face-Pytorch':
        transform = tr.Resize(256)
        return tds.face_dataset(config.training.data_dir, transform, condition=condition), tds.face_dataset(config.training.data_dir, transform, condition=condition)
    ...
    elif config.data.dataset == 'AFHQ-Pytorch': 
        transform = tr.Resize(256)
        return tds.afhq_dataset(config.training.data_dir, transform, condition=condition), tds.afhq_dataset(config.training.data_dir, transform, condition=condition)
    else:
